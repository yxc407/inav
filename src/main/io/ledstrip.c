...
static int currentColorIndex = 9;
...
static void applyLedFixedLayers(void)
{
    for (int ledIndex = 0; ledIndex < 24; ledIndex++) {
        ...
        switch (fn) {
            case LED_FUNCTION_COLOR:
                color = ledStripConfig() -> colors[currentColorIndex];
                break;
        }
    ...
    for (int ledIndex = 24; ledIndex < ledCounts.count; ledIndex++) {
        const ledConfig_t *ledConfig = &ledStripConfig()->ledConfigs[ledIndex];
        hsvColor_t color = *getSC(LED_SCOLOR_BACKGROUND);
...
void ledStripUpdate(timeUs_t currentTimeUs)
{
    ...
    if(IS_RC_MODE_ACTIVE(BOXLEDLOW) && !ledLock) {
        ledLock = true;
        if(++currentColorIndex > 13) currentColorIndex = 0;
        LEDSwitchUpdate(currentColorIndex);
    }
...
void LEDSwitchUpdate(int colorIndex)
{
