...
void failsafeApplyControlInput(void)
{
    ...
    for (int idx = 0; idx < 4; idx++) {
        ...
        //         break;
        // }
        if (failsafeProcedureLogic[failsafeState.activeProcedure].channelBehavior[idx] == FAILSAFE_CHANNEL_HOLD) {
            rcCommand[idx] = failsafeState.lastGoodRcCommand[idx];
        } else if (failsafeProcedureLogic[failsafeState.activeProcedure].channelBehavior[idx] == FAILSAFE_CHANNEL_NEUTRAL) {
